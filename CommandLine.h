...
class CommandLine{
    ...
    // check if the word "word", or a subpart of it, is found
    bool contains(const std::string& word) const;

    // returns information for the analysis
    const std::vector<std::string>& GetInfoForAnalysis() const;

    // list of all words passed in command line
    inline 
    const std::vector<std::string>& getAllInfo() const { return m_Words; }

    inline 
    std::string& getDefaultString() const { return m_DefaultString; }

    const std::string& getFileNameRAW() const;

    // function to get values given a keyword, using the following syntax:
    // keyWord=value. It takes keyword and return value
