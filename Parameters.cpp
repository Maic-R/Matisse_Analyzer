...
NameParser::NameParser(std::string fileName_RAW){
    ...
    // retrieve file name starting from the complete path
...
void NameParser::SetParameters(){
    ...
    // check that the last word is "_RAW.root"
    if( m_Token.back() != "_RAW.root" ){
        std::cerr << "File is not a RAW root file!\n";
