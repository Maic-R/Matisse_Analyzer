...
hMultiplicity_sector{nullptr}, hRow_pxl_sector{nullptr}, hCol_pxl_sector{nullptr},
...
void Cluster::Book(){
    ...
    Save( hSingle_pxl_cluster = new TH1F("hSingle_pxl_cluster", "Single pxl cluster distr", 100, 0, 700) );
...
void Cluster::Begin(const CommandLine& cl){
    ...
    // default value for cluster threshold = 4
    m_Clz_thr = (cl.contains("ClusterThr")) ? (cl.getValue("ClusterThr")) : 4;
    DBPRINT("Pixel threshold set to : ", m_Clz_thr);
...
void Cluster::Analyze(unsigned int iClz, unsigned int iSec){
    ...
    for(unsigned int k=0; k < Clz::MatrixSize; ++k){
        ...
        if( pulseHeight/noise > m_Clz_thr ){
            // update cluster mass
